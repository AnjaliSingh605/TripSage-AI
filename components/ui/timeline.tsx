...
export const Timeline = ({ data, trip_data }: { data: TimelineEntry[], trip_data: TripInfo }) => {
  ...
  return (
    // px-2 on mobile, md:px-10 kept as original
    <div
      className="w-full dark:bg-neutral-950 font-sans px-2 md:px-10"
      ref={containerRef}
    >
      {/* px-2 on mobile, md:px-8 lg:px-10 kept as original */}
      <div className="max-w-7xl mx-auto py-7 px-2 md:px-8 lg:px-10">
        <h2 className="text-lg md:text-4xl mb-4 font-bold dark:text-white max-w-4xl">
          Your Trip Itinerary from{" "}
          <strong className="text-[#444ce7]">{trip_data?.source}</strong> to{" "}
          <strong className="text-[#444ce7]">{trip_data?.destination}</strong> is Ready
        </h2>

        {/* flex-wrap added so it doesn't overflow on small screens */}
        <div className="flex flex-wrap gap-4 items-center mt-2">
          <div className="flex gap-2 items-center">
            <Calendar className="h-4 w-4 flex-shrink-0" />
            <h2 className="text-sm">{trip_data.duration}</h2>
          </div>
          <div className="flex gap-2 items-center">
            <Wallet2 className="h-4 w-4 flex-shrink-0" />
            <h2 className="text-sm">{trip_data.budget}</h2>
          </div>
          <div className="flex gap-2 items-center">
            <Users className="h-4 w-4 flex-shrink-0" />
            <h2 className="text-sm">{trip_data.group_size}</h2>
          </div>
        </div>
      </div>
